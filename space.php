...
// 获取用户信息
$user = userBySubdomain($sub);
if (!$user) {
    http_response_code(404);
    echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>404</title></head><body style="text-align:center;padding:60px 20px;font-family:sans-serif;color:#4a3f35;background:#f5f0eb;"><h1>用户不存在</h1><p>该用户可能已被删除或不存在</p><a href="/" style="color:#7A5C2D;text-decoration:none;border:1px solid #7A5C2D;padding:8px 24px;border-radius:30px;display:inline-block;margin-top:16px;">返回首页</a></body></html>';
    exit;
}
...
// 判断关注状态
$followStatus = $me ? getFollowStatus($me['id'], $user['id']) : 'not_following';
...
$posts = getUserPosts($user['id'], $page, 20, $me['id'] ?? null);
...
$s->execute([$user['id']]);
$totalPosts = $s->fetchColumn();
$hasMore = ($page * 20) < $totalPosts;

$title = e($user['display_name'] ?: $user['username']) . ' 的主页 - 瑞格米';
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<div class="main-wrapper">
    <main class="main-content">
        ...
        <div class="space-header">
            <div class="space-cover" style="background-image: url('<?php echo $user['header_bg'] ? '/uploads/headers/' . $user['header_bg'] : '/assets/default-header.jpg'; ?>');"></div>
            <div class="space-info">
                <img src="<?php echo getAvatarUrl($user['avatar']); ?>" class="space-avatar" onerror="this.src='/assets/default-avatar.png'">
                <div class="space-name"><?php echo e($user['display_name'] ?: $user['username']); ?></div>
                <div class="space-handle">@<?php echo e($user['subdomain']); ?></div>
                <?php if ($user['bio']): ?>
                    <div class="space-bio"><?php echo e($user['bio']); ?></div>
                <?php endif; ?>
                <div class="space-stats">
                    <span><strong><?php echo $userStats['posts']; ?></strong> 帖子</span>
                    <span><strong><?php echo $userStats['followers']; ?></strong> 粉丝</span>
                    <span><strong><?php echo $userStats['following']; ?></strong> 关注</span>
                </div>
                <?php if ($me && !$isOwnProfile): ?>
                    <button class="follow-btn <?php echo $followStatus === 'following' ? 'following' : ''; ?>" onclick="toggleFollow(<?php echo $user['id']; ?>, this)">
                        <?php echo $followStatus === 'following' ? '已关注' : '关注'; ?>
                    </button>
                <?php endif; ?>
                <?php if ($isOwnProfile): ?>
                    <a href="/settings" class="edit-profile-btn">编辑资料</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div id="post-feed">
            <?php if ($posts): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="end-indicator">还没有帖子</div>
            <?php endif; ?>
        </div>
        <?php if ($hasMore): ?>
            <div class="loading-indicator">
                <button class="btn-text" onclick="loadMoreUserPosts(<?php echo $user['id']; ?>, <?php echo $page; ?>)">加载更多</button>
            </div>
        <?php endif; ?>
...
function loadMoreUserPosts(userId, page) {
    if (loadingMore) return;
    loadingMore = true;
    var nextPage = page + 1;
    var status = document.querySelector('.loading-indicator');
    status.innerHTML = '<div class="spinner"></div>';
    fetch('/api/user_posts?user_id=' + userId + '&page=' + nextPage)
        .then(function(r) { return r.json(); })
        .then(function(data) {
            loadingMore = false;
            if (data.code === 200 && data.data.posts && data.data.posts.length > 0) {
                ...
                if (data.data.has_more) {
                    status.innerHTML = '<button class="btn-text" onclick="loadMoreUserPosts(' + userId + ', ' + nextPage + ')">加载更多</button>';
                } else {
        ...
        .catch(function() {
            loadingMore = false;
            status.innerHTML = '<button class="btn-text" onclick="loadMoreUserPosts(' + userId + ', ' + page + ')">重试</button>';
            showToast('加载失败，请重试');
        });
}
...
window.loadMoreUserPosts = loadMoreUserPosts;
